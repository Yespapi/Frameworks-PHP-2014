...
class MandatList extends Lister
{
    ...
    public function __construct($items = null)
    {
        ...
        if (!is_null($items)) {
            ...
            foreach ($items as $product) {
                $all_items &= $product instanceof Mandat;
            }
            if ($all_items) {
                $this->setItems($items);
    ...
    public function getTotalListSize($moderation_states = null)
    {
        try {
            $db = DB::get();
            $query = '	SELECT mandat_id
			   			FROM t_mandat
   						WHERE mandat_supprime = \'0\' ';

            if (!is_null($moderation_states))
                $query .= $moderation_states;

            $query .= ' ORDER BY mandat_id DESC';
    ...
    public function getSearchResults($moderation_states = null,$limit=null)
    {
        ...
        try {
            ...
            $query .= ' ORDER BY mandat_id DESC '.$limit_clause;
            ...
            while ($row = $db->fetch_assoc($result)) {
                $items[] = new Mandat($row['mandat_id']);
            }
            $this->setItems($items);
            return $items;
