...
class Assurance
{
    ...
    static function DeleteAssurance($array)
    {
        try {
            $db = DB::get();
   ...
   public function delete(){
    ...
    public function modifierAssurance()
    {
        try {
            $db = DB::get();
    ...
    public function set()
    {
        ...
        try {
            ...
            $result = $db->query($Req);
            $Assurance = $db->fetch_object($result);

            $this->Date = $Assurance->{self::$Table . '_date'};
            $this->Civilite = $Assurance->{self::$Table . '_civilite'};
            $this->Prenom = stripslashes($Assurance->{self::$Table . '_prenom'});
            $this->Nom = stripslashes($Assurance->{self::$Table . '_nom'});
            $this->Societe = stripslashes($Assurance->{self::$Table . '_societe'});
            $this->Email = stripslashes($Assurance->{self::$Table . '_email'});
            $this->Tel 		= stripslashes($Assurance->{self::$Table . '_tel'});
            $this->Idproduit = $Assurance->{self::$Table . '_idproduit'};
