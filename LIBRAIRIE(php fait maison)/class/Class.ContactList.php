...
class ContactList extends Lister
{
    ...
    public function __construct($items = null)
    {
        ...
        if (!is_null($items)) {
            ...
            foreach ($items as $product) {
                $all_items &= $product instanceof Contact;
            }
            if ($all_items) {
                $this->setItems($items);
    ...
    public function getSearchResults($moderation_states = null,$limit=null)
    {
        ...
        try {
            ...
            $query .= ' ORDER BY contact_id DESC '.$limit_clause;
            ...
            while ($row = $db->fetch_assoc($result)) {
                $items[] = new Contact($row['contact_id']);
            }
            $this->setItems($items);
            return $items;
