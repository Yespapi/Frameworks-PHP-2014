 ...
 * 
 * @package default
 * @author Aly <aly.ba2009@gmail.com> & professeur Mr Martin
 * @todo scripts les plus importants, de fonctions utilitaires d'intéraction avec la base de donnée
 * CRUD & SID qui sont utilisés dans les interfaces, visiteur et comptablees essentiallement.
 * Respecter les conventions de nommages en particulier des variables.
 * 
 */
$hote = "localhost";
$login = "userGsb";
...
function obtenirDetailFicheFrais($idCnx, $unMois, $unIdVisiteur) {
    ...
    $ligne = false;
    $requete = "select IFNULL(nbJustificatifs,0) as nbJustificatifs, Etat.id as idEtat, libelle as libelleEtat, dateModif, montantValide, idTypeVehicule
...
function ajouterFicheFrais($idCnx, $unMois, $unIdVisiteur) {
    $unMois = filtrerChainePourBD($unMois);
    // Modifie de la dernière fiche de frais du visiteur
    ...
    if (is_array($laDerniereFiche) && $laDerniereFiche['idEtat'] == 'CR') {
        ...
        $typeVehicule = $laDerniereFiche['idTypeVehicule'];
    } else {
        $typeVehicule = '4cve';
    }

    // Ajoute de la fiche de frais à l'état Créé
    $requete = "insert into FicheFrais (idVisiteur, mois, nbJustificatifs, montantValide, idEtat, dateModif, idTypeVehicule) values ('"
            . $unIdVisiteur
            . "','" . $unMois . "',0,NULL, 'CR', '" . date("Y-m-d") . "', '" . $typeVehicule . "')";
    mysql_query($requete, $idCnx);

    // Ajoute des éléments forfaitisés
...
function reporterLigneHorsForfait($idCnx, $unIdLigneHF) {
    mysql_query('CALL reporterLigneFraisHF(' . $unIdLigneHF . ');', $idCnx) or die(mysql_error());
}
...
/**
 * Retourne le type de véhicule d'un visiteur pour un mois donné. 
 * 
 * La fonction retourne l'id, le libellé, et l'indemnité correspondant au type
 * de véhicule du visiteur
 * d'id $idVisiteur pour le mois $mois    
 * @param resource $idCnx identifiant de connexion
 * @param string $unMois mois demandé (MMAAAA)
 * @param string $unIdVisiteur id visiteur  
 * @return array tableau associatif ou booléen false 
 */
function obtenirTypeVehicule($idCnx, $unMois, $unIdVisiteur) {
    $req = "select id, libelleType, indemniteKm from typeVehicule inner join ficheFrais on (ficheFrais.idTypeVehicule = typeVehicule.id) where idVisiteur='" . $unIdVisiteur . "' and mois='" . $unMois . "'";
    $idJeuTypeVehicule = mysql_query($req, $idCnx);
    $ligne = false;
    if ($idJeuTypeVehicule) {
        $ligne = mysql_fetch_assoc($idJeuTypeVehicule);
...
/**
 * Retourne la requete d'obtention de la liste des types de véhicules
 *
 * Retourne la requête d'obtention de la liste des types de véhicules (id, libelle et indemnité)
 * @return string $requete
 */
function obtenirReqListeTypesVehicules() {
    $requete = "select id, libelleType, indemniteKm from typevehicule order by id";
    return $requete;
}

/**
 * Modifie le type de vehicule d'une fiche de frais
 *
 * Met à jour le type de vehicule de la fiche de frais du visiteur $unIdVisiteur pour
 * le mois $unMois 
 * @param resource $idCnx identifiant de connexion
 * @param string $unIdVisiteur 
 * @param string $unMois mois sous la forme aaaamm
 * @param string $unTypeVehicule
 * @return void 
 */
function modifierTypeVehiculeFicheFrais($idCnx, $unMois, $unIdVisiteur, $unTypeVehicule) {
    $requete = "update FicheFrais set idTypeVehicule = '" . $unTypeVehicule . "' where idVisiteur ='" . $unIdVisiteur . "' and mois = '" . $unMois . "'";
    mysql_query($requete, $idCnx) or die(mysql_error());
    mysql_query("CALL repercuterChangementTypeVehicule('" . $unTypeVehicule . "','" . $unIdVisiteur . "','" . $unMois . "');", $idCnx) or die(mysql_error());
}