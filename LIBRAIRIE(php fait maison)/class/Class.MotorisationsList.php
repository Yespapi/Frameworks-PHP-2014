...
class MotorisationsList extends Lister {
		...
		try {
			...
			if(!empty($moderation_states)){
				...
				foreach($moderation_states as $key=>$val){
					...
					$search_clause .= ( ($key == 'nom') && ($val != '') )?' WHERE motorisations_nom LIKE \'%'.$db->escape($val).'%\'':'';
			...
			$query = '	SELECT motorisations_id
			   			FROM t_motorisations ';
			$query .= 	$search_clause;
   			$query .= ' ORDER BY motorisations_id DESC';
	...
	public function getSearchResults($moderation_states = '',$order = null){
		...
		try{
			...
			$order_by = ($order == null)?' ORDER BY motorisations_nom ASC':' ORDER BY motorisations_'.$order.' ASC';
			...
			while($row = $db->fetch_assoc($result)){
				$items[] = new Motorisations($row['motorisations_id']);
			}		
			$this->setItems($items);
			return $items;
