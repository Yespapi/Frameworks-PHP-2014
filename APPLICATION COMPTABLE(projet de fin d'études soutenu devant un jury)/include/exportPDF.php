...
class PDF extends FPDF {
    ...
    function tabFraisForfaits($bdd, $idMois, $idVisiteur) {
        ...
        $this->SetTextColor(0, 0, 0);
        $this->SetFont('Times', '', 12);
        $idJeuFraisForfait = $bdd->query("select libelle, quantite, montant, (quantite*montant) as total from LigneFraisForfait inner join FraisForfait on (FraisForfait.id = LigneFraisForfait.idFraisForfait) where idVisiteur='" . $idVisiteur . "' and mois='" . $idMois . "' and LigneFraisForfait.idFraisForfait != 'KM'");
        ...
        // Traitement spécifiques de l'indemnité kilométrique
        $idJeuIndemKm = $bdd->query("select libelle, quantite, indemniteKm, (quantite*indemniteKm) as total from FicheFrais inner join TypeVehicule on (FicheFrais.idTypeVehicule = TypeVehicule.id) inner join LigneFraisForfait on (FicheFrais.idVisiteur = LigneFraisForfait.idVisiteur and FicheFrais.mois = LigneFraisForfait.mois) inner join FraisForfait on (FraisForfait.id = LigneFraisForfait.idFraisForfait) where FicheFrais.idVisiteur='" . $idVisiteur . "' and FicheFrais.mois='" . $idMois . "' and LigneFraisForfait.idFraisForfait = 'KM'");
        while ($lgIndemKm = $idJeuIndemKm->fetch()) {
            $this->Cell(10);
            $this->Cell(50, 7, $lgIndemKm['libelle'], 1, 0, 'L', true);
            $this->Cell(40, 7, $lgIndemKm['quantite'], 1, 0, 'R', true);
            $this->Cell(40, 7, $lgIndemKm['indemniteKm'], 1, 0, 'R', true);
            $this->Cell(40, 7, $lgIndemKm['total'], 1, 0, 'R', true);
            $this->Ln();
        }
        $idJeuIndemKm->closeCursor();
    }
...
// Renommage du fichier (ce qui permet de masquer le nom réel du fichier sur le disque)
